...
import { grupoData as data } from "../../constants/sectionsData";
...
function InfoMonteperdido({}: Props) {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    document.title = `${data.seccionName} | GS Monteperdido 960 Parla`;
  }, [location.pathname]);
  ...
  return (
    <>
      ...
      <section className="section_scouters">
        ...
        <div className="section_scouters__container mx-auto flex-wrap flex items-center max-w-[80dvw] justify-center gap-[7%] px-10 pb-10">
          {data.scouters.monis.map((moni, index) => (
            ...
            >
              <div className="section_scouters__moni__img h-[220px] flex items-center justify-center">
                <img
                  src={moni.foto}
                  alt={`Foto de ${moni.nombre2 ? moni.nombre2 : moni.nombre}`}
                  className="object-contain h-4/5 w-4/5 bg-green-300"
                />
              </div>
      ...
      <section className="section_unete bg-white bg-[radial-gradient(100%_50%_at_50%_0%,rgba(0,163,255,0.13)_0,rgba(0,163,255,0)_50%,rgba(0,163,255,0)_100%)]">
        <SectionTitle
          color={data.color}
